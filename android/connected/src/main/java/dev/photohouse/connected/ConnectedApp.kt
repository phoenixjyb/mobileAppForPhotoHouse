        ...
        Surface(Modifier.fillMaxSize()) {
            ...
            LazyColumn(Modifier.fillMaxSize().safeDrawingPadding().testTag("connected-screen"), state = scroll,
                ...
                item { Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    Text(t("Connected development build", "联网开发版本"))
                    ...
                    when {
                        ...
                        state.session == null -> item { key(state.generation) { AdmissionForm(store, state, words) } }
                            ...
                            item { FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                OutlinedButton(onClick = store::libraries) { Text(t("Libraries", "资料库")) }
                                OutlinedButton(onClick = store::logout) { Text(t("Sign out", "退出登录")) }
                            } }
                            when {
                                state.library == null -> {
                                    ...
                                    items(state.session!!.memberships, key = { it.library_id }) { membership ->
                                        Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(16.dp)) {
                                            Text(membership.library_id, style = MaterialTheme.typography.titleMedium)
                                    ...
                                    state.gallery?.let { gallery ->
                                        ...
                                        items(gallery.items.chunked(columns)) { row -> Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                                            row.forEach { asset -> Card(onClick = { store.openAsset(asset) }, modifier = Modifier.weight(1f)) {
                                                ...
                                                Text(asset.taken_at ?: t("Date unknown", "日期未知"), Modifier.padding(12.dp))
                                            } }
                                        ...
                                        item {
                                            ...
                                            FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                                Button(onClick = { store.loadPage(gallery.page - 1) }, enabled = !state.busy && gallery.page > 1) { Text(t("Previous", "上一页")) }
                                                Button(onClick = { store.loadPage(gallery.page + 1) }, enabled = !state.busy && gallery.page < 100000 && gallery.page.toLong() * gallery.page_size < gallery.total) { Text(t("Next", "下一页")) }
...
@Composable private fun AdmissionForm(store: ConnectedStore, state: LiveState, words: Words) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        Text(words.t("Phone is an unverified login label. Use your country code. Registration requires an owner-issued invitation.", "手机号是未经验证的登录标识，请包含国家码。注册需要所有者发出的邀请。"))
...
@Composable private fun Preview(asset: Asset, bytes: ByteArray?, words: Words) {
    ...
    if (image == null) Box(Modifier.fillMaxWidth().heightIn(min = 120.dp).padding(16.dp)) { Text(words.t("Preview unavailable", "预览不可用")) }
    else Image(image, contentDescription = words.t("Photo ${asset.id}", "照片 ${asset.id}"), contentScale = ContentScale.Crop, modifier = Modifier.fillMaxWidth().aspectRatio(4f/3))
}
